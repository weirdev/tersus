...
1. Element-level facts about lists
    Contracts can only state sizes and the element just written today: `push` and `set` only say which element they wrote (`get(return, size(list)) = x` and `get(return, index) = x`, see `examples/element_facts.tersus`), and there is no way to say anything about the other elements or to say something about every index, so `pairSums` (examples/parallel_sum.tersus) cannot state that each result element is the sum of the corresponding pair
    Give `push` and `set` frame facts, that the other elements are unchanged (`get(return, j) = get(list, j)` for `j < size(list)`, and for `set` every `j` other than `index`); this needs guarded facts and a `!=` relation or ordered guards checked with `checkRel`, for example as axioms applied with `rewrite`
    A bounded quantified fact, usable in output contracts and loop invariants: `forall j where j >= 0, j < i: P` with `forall j in 0..i: P` as shorthand (half-open range). Facts are instantiated at the `get(L, t)` terms the closure contains whose guard is entailed, and proving one introduces a fresh index with the guard assumed
    Carrying such an invariant through a loop needs index arithmetic (item 7)
...
7. Proof transformation v2
    ...
    Every index counter in a list loop (`parallel_sum`, `build_list`, `update_list`) needs trusted axioms for `i >= 0`, `i <= n` and `i = n` after the loop
...
9. Test against motivating example cases (safe access to lize of size known at runtime, parallel iteration of lists, provably safe doubly linked list)
    ...
    The doubly linked list needs lists that can be updated (`set`, done, see examples/update_list.tersus) plus element-level facts (item 1) to say what each link points to
