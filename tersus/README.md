...
1. Element-level facts about lists
    Contracts can only state sizes today: `push`, `set` and `get` say nothing about elements, and there is no way to say something about every index, so `pairSums` (examples/parallel_sum.tersus) cannot state that each result element is the sum of the corresponding pair
    Give `push` output facts (`get(return, size(list)) = x`, and `get(return, j) = get(list, j)` for `j < size(list)`)
    Give `set` output facts (`get(return, index) = x`, and `get(return, j) = get(list, j)` for every other in-range `j`)
